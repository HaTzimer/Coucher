...
- Keep WebApi transport concerns in `Coucher.WebApi`, reusable auth/session logic in `Coucher.Lib`, and shared models/interfaces/configuration keys in `Coucher.Shared`.
- Put cache provider implementations under `Coucher.Lib/DAL/Cache`.
- Put cache provider contracts under `Coucher.Shared/Interfaces/DAL/Providers`.
...
2. Add a Redis-backed cache provider in `Coucher.Lib/DAL/Cache` with its interface in `Coucher.Shared/Interfaces/DAL/Providers`.
