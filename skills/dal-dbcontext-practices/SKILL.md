...
- Create dedicated internal DAL projection models for partial SQL reads under `Coacher.Shared/Models/DAL/Internal`.
...
- In providers, use `IDbContextFactory<CoacherDbContext>` as the DbContext access pattern.
...
- Keep provider constructors minimal and assign only `_dbContextFactory`, for example:
  ...
  `public UserNotificationProvider(IDbContextFactory<CoacherDbContext> dbContextFactory) { _dbContextFactory = dbContextFactory; }`
...
- Reject provider code that injects `CoacherDbContext` directly instead of using `IDbContextFactory<CoacherDbContext>`.
