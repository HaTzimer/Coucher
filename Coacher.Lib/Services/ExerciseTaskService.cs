...
public sealed class ExerciseTaskService : IExerciseTaskService
{
    ...
    public async Task DeleteExerciseTaskDependenciesAsync(
        Guid taskId,
    ...
    )
    {
        ...
        var currentUserId = await _currentUserService.GetRequiredCurrentUserIdAsync(cancellationToken);
        var parsedDependencyIds = dependencyIds.Select(item => ParseGuidString(item, "DependencyId")).Distinct().ToList();
        if (parsedDependencyIds.Count == 0)
            return;

        await _repository.DeleteTaskDependenciesAsync(taskId, parsedDependencyIds, cancellationToken);

        _logger.Info("Exercise task dependencies removed", new Dictionary<string, object>
        {
            { "userId", currentUserId },
            { "taskId", taskId },
            { "count", parsedDependencyIds.Count },
            { "result", "success" }
        });
    }

    public async Task<List<ExerciseTaskResponsibleUser>> AddExerciseTaskResponsibleUsersAsync(
        Guid taskId,
        List<string> userIds,
        CancellationToken cancellationToken = default
    )
    {
        ...
        if (parsedUserIds.Count == 0)
            return new List<ExerciseTaskResponsibleUser>();

        var now = DateTime.UtcNow;
        var entities = parsedUserIds.Select(parsedUserId => new ExerciseTaskResponsibleUser
            {
                Id = Guid.NewGuid(),
                TaskId = taskId,
                UserId = parsedUserId,
            ...
            .ToList();
        var createdEntities = await _repository.CreateExerciseTaskResponsibleUsersAsync(
            entities,
            cancellationToken
        );

        _logger.Info("Exercise task responsible users added", new Dictionary<string, object>
        {
            { "userId", currentUserId },
            { "taskId", taskId },
            { "count", createdEntities.Count },
    ...
    )
    {
        ...
        var currentUserId = await _currentUserService.GetRequiredCurrentUserIdAsync(cancellationToken);
        var parsedResponsibilityIds = responsibilityIds
            .Select(item => ParseGuidString(item, "ResponsibilityId"))
            .Distinct()
            .ToList();
        if (parsedResponsibilityIds.Count == 0)
            return;

        await _repository.DeleteExerciseTaskResponsibleUsersAsync(
            taskId,
            parsedResponsibilityIds,
            cancellationToken
        );
        ...
        _logger.Info("Exercise task responsible users bulk removed", new Dictionary<string, object>
        {
            { "userId", currentUserId },
            { "taskId", taskId },
            { "count", parsedResponsibilityIds.Count },
            { "result", "success" }
        });
    }
