using Coacher.Shared.Models.WebApi.Requests.Admin;
using Microsoft.OpenApi.Any;
using Microsoft.OpenApi.Models;
using Swashbuckle.AspNetCore.SwaggerGen;

namespace Coacher.WebApi.Swagger;

public sealed class TaskTemplateCreateRequestSchemaFilter : ISchemaFilter
{
    public void Apply(OpenApiSchema schema, SchemaFilterContext context)
    {
        if (context.Type == typeof(CreateTaskTemplateRequest))
        {
            ConfigureChildrenSchema(context, schema);
            schema.Example = BuildRootExample();

            return;
        }

        if (context.Type == typeof(CreateTaskTemplateChildRequest))
        {
            ConfigureChildrenSchema(context, schema);
            schema.Example = BuildChildExample("child-b", "Child B", 1, new[] { "child-a" });
        }
    }

    private static void ConfigureChildrenSchema(SchemaFilterContext context, OpenApiSchema schema)
    {
        if (!schema.Properties.TryGetValue("children", out var childrenSchema))
            return;

        _ = context.SchemaGenerator.GenerateSchema(typeof(CreateTaskTemplateChildRequest), context.SchemaRepository);
        childrenSchema.Type = "array";
        childrenSchema.Items = new OpenApiSchema
        {
            Reference = new OpenApiReference
            {
                Type = ReferenceType.Schema,
                Id = nameof(CreateTaskTemplateChildRequest)
            }
        };
    }

    private static OpenApiObject BuildRootExample()
    {
        return new OpenApiObject
        {
            ["templateKey"] = new OpenApiString("root-template"),
            ["seriesId"] = new OpenApiString("3fa85f64-5717-4562-b3fc-2c963f66afa6"),
            ["categoryId"] = new OpenApiString("3fa85f64-5717-4562-b3fc-2c963f66afb0"),
            ["name"] = new OpenApiString("Root Template"),
            ["description"] = new OpenApiString("Top level task template"),
            ["notes"] = new OpenApiString("Optional notes"),
            ["defaultWeeksBeforeExerciseStart"] = new OpenApiInteger(2),
            ["influencerIds"] = new OpenApiArray
            {
                new OpenApiString("3fa85f64-5717-4562-b3fc-2c963f66afc1")
            },
            ["children"] = new OpenApiArray
            {
                BuildChildExample(
                    "child-a",
                    "Child A",
                    1
                ),
                BuildChildExample(
                    "child-b",
                    "Child B",
                    1,
                    new[] { "child-a" }
                )
            }
        };
    }

    private static OpenApiObject BuildChildExample(
        string templateKey,
        string name,
        int defaultWeeksBeforeExerciseStart,
        IEnumerable<string>? dependsOnTemplateKeys = null
    )
    {
        var example = new OpenApiObject
        {
            ["templateKey"] = new OpenApiString(templateKey),
            ["seriesId"] = new OpenApiString("3fa85f64-5717-4562-b3fc-2c963f66afa6"),
            ["categoryId"] = new OpenApiString("3fa85f64-5717-4562-b3fc-2c963f66afb0"),
            ["name"] = new OpenApiString(name),
            ["description"] = new OpenApiString("Child task template description"),
            ["notes"] = new OpenApiString("Optional child task template notes"),
            ["defaultWeeksBeforeExerciseStart"] = new OpenApiInteger(defaultWeeksBeforeExerciseStart)
        };

        example["influencerIds"] = new OpenApiArray
        {
            new OpenApiString("3fa85f64-5717-4562-b3fc-2c963f66afc1")
        };

        if (dependsOnTemplateKeys is not null)
        {
            var dependsOnKeysArray = new OpenApiArray();
            foreach (var dependsOnTemplateKey in dependsOnTemplateKeys)
            {
                dependsOnKeysArray.Add(new OpenApiString(dependsOnTemplateKey));
            }

            example["dependsOnTemplateKeys"] = dependsOnKeysArray;
        }

        return example;
    }
}
